...
def read_dump(lang_code,file_dict):
    ...
    for _, elem in tqdm(ET.iterparse(filepath, events=("end",))):
        if elem.tag.endswith('text'):
            ...
            yield titles[-1],meta[-1],articles[-1]
            titles,meta,articles=[],[],[]
            buffer=[]
        else:
            try:
...
print(file_dict)
file_dict={'en':'enwiki-20230301-pages-articles-multistream.xml.bz2',
            'te': 'tewiki-20230301-pages-articles-multistream.xml.bz2',  }

def save_article(i,j,count=0):
    j = j.lower()
    ...
    art_df.to_csv(f"clean-dumps/sentence_data_ent_{lang_code}.csv",mode=mode,header=header,index=False)
...
for i,(lang_code,val) in enumerate(file_dict.items()):
    ...
    count=0
    gen = read_dump(lang_code,file_dict)
    while True:
        try:
            titles,meta,articles = next(gen)
            save_article(articles,titles,count)
            count+=1
